...
configure-libsrtp:
	git clone http://github.com/cisco/libsrtp
	cd libsrtp && git checkout v1.5.0 && CFLAGS="-fPIC" && ./configure --enable-pic && make && sudo make install
	cd ..

configure-duobango:
	git clone https://github.com/DuobangoTelecom/duobango.git ~/duobango
...
configure-webrtc:
	...
	cd ~/webrtc2sip && export PREFIX=/usr/local/opt/webrtc2sip && ./autogen.sh && ./configure  --with-doubango=/usr/local CFLAGS='-lpthread' LDFLAGS='-ldl' LIBS='-ldl' --prefix=$PREFIX && make clean && make && sudo make install cp -f ./config.xml $PREFIX/sbin/config.xml
	cd ..
